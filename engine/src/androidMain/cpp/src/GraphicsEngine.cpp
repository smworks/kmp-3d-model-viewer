...
namespace engine
{
	CGraphicsEngine::CGraphicsEngine() = default;
	CGraphicsEngine::~CGraphicsEngine() = default;

	bool CGraphicsEngine::bInitialize(const SInitConfig& sConfig)
	{
		...
		{
			LOGE("GraphicsEngine initialization failed: invalid native window or asset manager");
			return false;
		}
		...
		const VkExtent2D displayExtent = resolveDisplayExtent(g.swapchainExtent, g.surfaceTransform);
		LOGI("GraphicsEngine initialized w=%u h=%u swapchain=%ux%u display=%ux%u aspect=%.3f transform=%s (%u)",
	...
	void CGraphicsEngine::InitializeResources()
	{
	}

	void CGraphicsEngine::Resize(const SFrameConfig& sFrame)
	{
		if (!g.initialized)
		{
			return;
	...
	void CGraphicsEngine::RenderFrame(const SCameraState&)
	{
	...
	void CGraphicsEngine::LoadModel(const SModelDescription& sModel)
	{
	...
	void CGraphicsEngine::UpdateModelTransform(const SModelDescription& sModel)
	{
		std::lock_guard<std::mutex> oGuard(g_stateMutex);
	...
	void CGraphicsEngine::RemoveModel(int64_t llModelId)
	{
		std::lock_guard<std::mutex> oGuard(g_stateMutex);
	...
	void CGraphicsEngine::TranslateModel(int64_t llModelId, float fX, float fY, float fZ)
	{
		if (!g.initialized)
		{
			return;
	...
	void CGraphicsEngine::ScaleModel(int64_t llModelId, float fScale)
	{
		if (!g.initialized)
		{
			return;
	...
	void CGraphicsEngine::RotateModel(int64_t llModelId, float fX, float fY, float fZ)
	{
		if (!g.initialized)
		{
			return;
	...
	void CGraphicsEngine::MoveCamera(float fDelta)
	{
		if (!g.initialized) return;
		g.camera.move(fDelta);
	}

	void CGraphicsEngine::SetCameraPosition(const SCameraState& sCamera)
	{
		if (!g.initialized) return;
	...
	void CGraphicsEngine::SetCameraRotation(const SCameraState& sCamera)
	{
		if (!g.initialized) return;
	...
	void CGraphicsEngine::RotateCamera(float fYaw, float fPitch, float fRoll)
	{
		if (!g.initialized) return;
	...
	{
		g_javaVm = pVm;
		g_engineApiClass = engineApiClass;
		g_loadFileMethod = loadFileMethod;
	}
}
